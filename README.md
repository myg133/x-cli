...
x-cli 通过 npm 分发二进制，版本号以 `packages/x-cli-npm/package.json` 为**单一事实源**。
...
3. **推送**：`git push --tags` → GitHub Actions release workflow 触发
4. **CI 自动**：构建 3 平台二进制 → 发布 npm 平台子包 → 发布 npm 主包
